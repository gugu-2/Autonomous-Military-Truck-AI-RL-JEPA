...
class VLAReasoner:
    ...
    def process_scene(self, image: Image.Image, telemetry: Dict[str, float], hazard_energy: float) -> Optional[Dict[str, Any]]:
        """
        ...
        if intent:
            logger.info(f"VLA Reasoning output: Action={intent['action']}, Reason={intent['reason']}")
