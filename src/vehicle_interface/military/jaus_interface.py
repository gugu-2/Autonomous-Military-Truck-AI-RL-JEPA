...
class JausInterface:
    ...
    def __init__(self, local_address: JausAddress, remote_address: JausAddress, udp_port: int = 3794):
    ...
    def _build_jaus_header(self, msg_type: int, data_len: int) -> bytes:
        ...
        header = struct.pack('<B', properties) + dest + src + struct.pack('<I', data_len) + struct.pack('<H', self.seq_num)
    ...
    def send_set_desired_travel_speed(self, speed_ms: float):
        ...
        msg = self._build_jaus_header(JausMessageType.SET_DESIRED_TRAVEL_SPEED, len(payload)) + payload
        self._send_udp(msg)
