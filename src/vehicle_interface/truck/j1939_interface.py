...
class J1939Interface:
    ...
    def _encode_j1939_frame(
        self, pgn: int, data: bytes, priority: int = 6, destination_address: int = 0xFF
    ) -> can.Message:
    ...
    def _decode_j1939_frame(self, msg: can.Message) -> J1939Message | None:
    ...
    def send_tsc1_engine_speed_request(
        self, target_speed_rpm: float, destination_address: int = 0x00
    ):
        ...
        msg = self._encode_j1939_frame(
            J1939PGN.PGN_TSC1, bytes(data), priority=3, destination_address=destination_address
        )
    ...
    def request_vehicle_speed(self) -> float | None:
