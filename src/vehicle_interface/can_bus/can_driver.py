...
class CANDriver:
    ...
    def __init__(self, channel: str = 'can0', bustype: str = 'socketcan', bitrate: int = 500000, timeout: float = 0.5):
    ...
    def _init_bus(self):
        try:
            ...
            logger.info(f"Initialized CAN bus {self.channel} ({self.bustype}) at {self.bitrate} bps")
    ...
    def start(self):
        ...
        self._init_bus()
        self._running = True
        self._rx_thread = threading.Thread(target=self._receive_loop, name="CAN_Rx_Thread", daemon=True)
    ...
    def stop(self):
        ...
        if self.bus:
            self.bus.shutdown()
            self.bus = None
        logger.info(f"CANDriver stopped. TX: {self.tx_count}, RX: {self.rx_count}, ERR: {self.error_count}")
