...
class DrivingPolicy:
    ...
    """
    def __init__(self, agent: DreamerV3Agent, world_model_wrapper: WorldModelRLWrapper, safety_interlock: SafetyInterlock, device: str = 'cuda'):
        self.agent = agent
    ...
    @torch.no_grad()
    def get_action(self, camera_frames: torch.Tensor, lidar_bev: torch.Tensor = None, hazard_map: torch.Tensor = None) -> Dict[str, float]:
        """
        ...
        raw_action, _ = self.agent.policy(obs_embed, self.prev_state, self.prev_action, explore=False)
        ...
        if hazard_map is not None:
            ...
            if max_hazard > 0.70:
                decoded_action = self.safety.trigger_emergency_brake(decoded_action, reason=f"Hazard E={max_hazard:.2f}")
