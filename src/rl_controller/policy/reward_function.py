...
from rl_controller.policy.action_space import VehicleMode, DrivingAction
...
class RewardFunction:
    def __init__(self, config: Dict[str, Any], vehicle_mode: VehicleMode):
        ...
        self.weights = config.get('weights', {}).get(vehicle_mode.name.lower(), {
            'safety': 1.0,
            'progress': 0.5,
            'comfort': 0.1,
            'traffic': 0.5,
    ...
    def compute(self, state: UnifiedWorldState, action: DrivingAction, next_state: UnifiedWorldState, 
                hazard_energy: float, route_progress: float, is_collision: bool) -> RewardBreakdown:
