...
class ImaginationTrainer:
    ...
    def __init__(self, agent: nn.Module, config: Dict[str, Any]):
        ...
        # Optimizers
        self.opt_rssm = torch.optim.AdamW(self.agent.rssm.parameters(), lr=config.get('rssm_lr', 1e-4))
        self.opt_actor = torch.optim.AdamW(self.agent.actor.parameters(), lr=config.get('actor_lr', 3e-5))
        self.opt_critic = torch.optim.AdamW(self.agent.critic.parameters(), lr=config.get('critic_lr', 3e-5))
        
        self.scaler = torch.cuda.amp.GradScaler(enabled=config.get('use_amp', True))
        
    def _compute_lambda_returns(self, rewards: torch.Tensor, values: torch.Tensor, continues: torch.Tensor) -> torch.Tensor:
        """
        ...
        for t in reversed(range(self.horizon)):
            ret = rewards[:, t] + self.gamma * continues[:, t] * (
                (1 - self.lmbda) * values[:, t + 1] if t + 1 < self.horizon else 0.0 +
                self.lmbda * last_val
            )
            returns[:, t] = ret
            last_val = ret
    ...
    def train_step(self, obs: torch.Tensor, actions: torch.Tensor, rewards: torch.Tensor, continues: torch.Tensor) -> Dict[str, float]:
        """
        ...
        with torch.cuda.amp.autocast(enabled=self.config.get('use_amp', True)):
            ...
            imag_features, imag_actions, imag_values = self.agent.imagine_trajectory(start_state, self.horizon)
