...
class WorldModelRLWrapper(nn.Module):
    ...
    def encode_observation(
        self, camera_frames: torch.Tensor, lidar_bev: torch.Tensor = None
    ) -> torch.Tensor:
        """
        ...
        with torch.no_grad():
            ...
            if hasattr(self.jepa_model, "encode_for_rl"):
                ...
                if hasattr(self.jepa_model, "encoder"):
                    ...
                    embed = torch.randn(
                        camera_frames.size(0), self.rssm.embed_dim, device=camera_frames.device
                    )
        return embed

    def forward(
        self, obs: torch.Tensor, prev_state: dict[str, torch.Tensor], prev_action: torch.Tensor
