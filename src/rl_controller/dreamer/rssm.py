...
class RSSM(nn.Module):
    ...
    def observe(self, embed: torch.Tensor, action: torch.Tensor, state: RSSMState) -> Tuple[RSSMState, RSSMState]:
        ...
        posterior = RSSMState(deter=prior.deter, stoch=stoch, logits=logits, mean=torch.zeros_like(stoch), std=torch.ones_like(stoch))
    ...
    def imagine(self, action: torch.Tensor, state: RSSMState) -> RSSMState:
        ...
        logits = logits.view(-1, self.stoch_dim, self.num_classes)
        stoch = self._straight_through_sample(logits)
        return RSSMState(deter=deter, stoch=stoch, logits=logits, mean=torch.zeros_like(stoch), std=torch.ones_like(stoch))
    ...
    def kl_loss(self, post_logits: torch.Tensor, prior_logits: torch.Tensor, free_nats: float = 3.0) -> torch.Tensor:
        """Computes KL divergence with free nats clipping."""
        post = torch.distributions.Independent(torch.distributions.OneHotCategorical(logits=post_logits), 1)
        prior = torch.distributions.Independent(torch.distributions.OneHotCategorical(logits=prior_logits), 1)
