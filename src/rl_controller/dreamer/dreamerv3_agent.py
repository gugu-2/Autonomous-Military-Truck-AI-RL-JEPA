...
class DreamerV3Agent(nn.Module):
    ...
    def policy(self, obs_embed: torch.Tensor, prev_state: Dict[str, torch.Tensor], prev_action: torch.Tensor, explore: bool = False) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
    ...
    def imagine_trajectory(self, start_state: Dict[str, torch.Tensor], horizon: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
