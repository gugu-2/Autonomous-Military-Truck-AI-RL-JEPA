...
from src.sensor_fusion.fusion.unified_world_state import UnifiedWorldState, EgoPose, SensorHealthStatus
        ...
        with self.lock:
            ...
            for sensor, last_update in self.sensor_health.last_update_times.items():
                if current_time - last_update > 1.0: # 1 second timeout
                    if sensor == "camera": self.sensor_health.camera_ok = False
                    if sensor == "lidar": self.sensor_health.lidar_ok = False
                    if sensor == "radar": self.sensor_health.radar_ok = False
                    if sensor == "gps_imu": 
