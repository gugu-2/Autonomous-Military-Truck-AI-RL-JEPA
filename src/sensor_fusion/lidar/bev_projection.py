...
class BEVProjector:
    ...
    def _remove_ground_plane(self, points: np.ndarray, distance_threshold: float = 0.2) -> np.ndarray:
    ...
    def _voxelize(self, points: np.ndarray):
        ...
        valid_mask = (x_bins >= 0) & (x_bins < self.grid_size) & (y_bins >= 0) & (y_bins < self.grid_size)
