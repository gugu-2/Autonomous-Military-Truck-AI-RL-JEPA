...
from .imagination_engine import DrivingAction, ImaginationEngine, LatentState
...
class JEPAWorldModel(nn.Module):
    def __init__(self, config: dict):
        ...
        self.target_encoder = EMATargetEncoder(
            online_encoder=self.online_encoder, momentum=config.get("ema_momentum", 0.996)
        )
        ...
        self.imagination_engine = ImaginationEngine(
            jepa_predictor=self.predictor, hazard_computer=self.hazard_computer, config=config
        )
    ...
    def imagine_and_filter(
        self, latent: LatentState, actions: list[DrivingAction]
    ) -> list[DrivingAction]:
