...
class ImaginationEngine:
    ...
    def imagine(
        self, current_latent: LatentState, candidate_actions: list[DrivingAction]
    ) -> ImaginationResult:
        """
        ...
        context_batch = current_latent.tokens.repeat(
            num_candidates, 1, 1
        )  # (B*num_candidates, N, D)

        action_vectors = torch.stack(
            [a.vector for a in candidate_actions], dim=0
        )  # (num_candidates, action_dim)
